...
create_datasets <- function(dat) {
  ...
  dat_filtered <- dat %>% filter(UVB > 10 & SO2 > 0.1) # filtered data
...
# Beijing data
path <- "data/beijing_dataset.csv"
fit_path <- "/scratch/dongelr1/susannar/kesa2024/model_results/fitted_models_beijing.rds"
score_df_path <- "/scratch/dongelr1/susannar/kesa2024/model_results/score_df_beijing.rds"

# Drop sector, there was only one sector
dat <- read.csv(path, stringsAsFactors = FALSE) %>% drop_na() %>% dplyr::select(-Time, -sector)
