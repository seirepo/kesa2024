...
train_models_with_outlier_filtering <- function() {
  ...
  path_h <- "/scratch/dongelr1/susannar/kesa2024/results2025/hyytiala"
  subd_h <- "final_fits"
  ...
  path_b <- "/scratch/dongelr1/susannar/kesa2024/results2025/beijing"
  subd_b <- "final_fits"
...
train_models_with_param_subset <- function() {
  ...
  files_hyy <- list.files(path = data_dir_hyy, pattern = "uvb_so2_filtered.csv", full.names = TRUE)
  files_bei <- list.files(path = data_dir_bei, pattern = "unfiltered.csv", full.names = TRUE)
  
  features_h <- c("SA_cm3", "UVB", "temp_K", "relative_humidity", "SO2", "CS_rate")
  features_b <- c("SA_cm3", "UVB", "temp_K", "SO2", "O3", "relative_humidity") #"relative_humidity", "SO2", "O3")
  
  #path_h <- "/scratch/dongelr1/susannar/kesa2024/results2025/hyytiala"
  #subd_h <- "uvb_so2_rh_temp_cs"
  ...
  #            lc_target = file.path(path_h, "learning_curves", subd_h),
  #            p_val = p_val)
  #print(paste("Models for Hyytiälä fitted with features: ", paste(features_h, collapse = ", ")))

  path_h <- "/scratch/dongelr1/susannar/kesa2024/results2025/hyytiala"
  subd_h <- "final_model_rerun"
  
  path_b <- "/scratch/dongelr1/susannar/kesa2024/results2025/beijing"
  subd_b <- "final_model_rerun"
  ...
  purrr::map(files_hyy, fit_models,
             ...
             lc_target = file.path(path_b, "learning_curves", subd_b),
             p_val = p_val)
  print("Hyytiälä model fitted with final features")
  ...
  purrr::map(files_bei, fit_models,
             ...
             lc_target = file.path(path_b, "learning_curves", subd_b),
             p_val = p_val)
  print("Beijing model fitted with final features (same as Hyytiälä)")
  
  subd_b <- "final_model_own_feats"
  purrr::map(files_bei, fit_models,
             features = features_b,
             exclude_features = FALSE,
             score_target = file.path(path_b, "scores", subd_b),
  ...
  print(paste("Beijing model fitted with features: ", paste(features_b, collapse = ", ")))
...
train_models_with_cum_subset <- function() {
  #data_dir_hyy <- "data/hyytiala/preprocessed"
  data_dir_bei <- "/scratch/dongelr1/susannar/kesa2024/data/beijing/preprocessed"
  
  #files_hyy <- list.files(path = data_dir_hyy, pattern = "unfiltered.csv|uvb_so2_filtered.csv", full.names = TRUE)
  files_bei <- list.files(path = data_dir_bei, pattern = "unfiltered.csv", full.names = TRUE)
  
  #features_h <- c("UVB", "SO2", "relative_humidity", "temp_K", "CS_rate")
  #subd_feats_h <- c("uvb", "so2", "rh", "temp", "cs")
  features_b <- c("UVB", "temp_K", "SO2", "O3", "relative_humidity")
  subd_feats_b <- c("uvb", "temp", "so2", "o3", "rh")
  
  #path_h <- "/scratch/dongelr1/susannar/kesa2024/results2025/hyytiala"
  path_b <- "/scratch/dongelr1/susannar/kesa2024/results2025/beijing"
  
  for (i in seq_along(features_b)) {
    subd <- paste(subd_feats_b[1:i], collapse = "_")
    #feats_h <- c("SA_cm3", features_h[1:i])
    feats_b <- c("SA_cm3", features_b[1:i])
    ...
    #print(paste("Models fit for Hyytiälä using data ", paste(feats_h, collapse = ", ")))
    ...
    print(paste("Models fit for Beijing using data ", paste(feats_b, collapse = ", ")))
  }
